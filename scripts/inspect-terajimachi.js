...
try {
  ...
  const uchiwake = wb.Sheets["内訳1"] || wb.Sheets["内訳"] || wb.Sheets["内訳(工事)"];
  if (uchiwake) {
    const data = XLSX.utils.sheet_to_json(uchiwake, { header: 1, defval: "" });
    console.log("\n内訳 rows:", data.length);
    data.slice(0, 25).forEach((r, i) =>
      console.log(i, (r[1] || "").toString().slice(0, 40), "|", (r[5] || "").toString().slice(0, 30))
    );
  } else {
    console.log("内訳 not found");
  }
  const daika = wb.Sheets["代価1"] || wb.Sheets["代価"] || wb.Sheets["代価(工事)"];
  if (daika) {
