...
## 3-4. データ保護SQL（商業向け・空上書き防止）

1. **SQL Editor** で `supabase/data_protection.sql` を実行（必須）
2. これにより以下が有効になります:
   - 案件データの空上書き・急減を DB 側で拒否
   - 保存前の自動スナップショット（`pre_write`）
   - 復元 API 用関数・古いバックアップ整理
3. アプリ側は日次 cron で全社バックアップも作成します
4. 復元は「設定」画面のバックアップ一覧から行えます
